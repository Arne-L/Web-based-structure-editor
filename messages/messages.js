...
import { Statement, HoleTkn } from "../syntax-tree/ast";
...
class CodeHighlight {
     ...
     */
    callbacks;
    constructor(editor, codeToHighlight) {
...
export class ConstructHighlight extends CodeHighlight {
    constructor(editor, codeToHighlight, rgbColour) {
    ...
    moveToConstructPosition() {
        ...
        if (this.selection.startColumn != newSelection.startColumn) {
            ...
            this.selection = newSelection;
            this.domElement.style.left = `${this.domElement.offsetLeft +
                diff * this.editor.computeCharWidthInvisible(this.selection.startLineNumber)}px`;
        }
    }
    /**
...
export class InlineMessage extends CodeHighlight {
    ...
    warningTxt;
    textElement;
    constructor(editor, code, warningTxt, index = -1) {
        super(editor, code);
...
export class HoverMessage extends InlineMessage {
     ...
     */
    contentEl;
    constructor(editor, code, warningText, highlightColour, index = -1) {
    ...
    createDomElement() {
        ...
        this.domElement.style.left = `${(currentLinePosition.column + 2) * this.editor.computeCharWidthGlobal() + 10}px`;
    ...
    moveToConstructPosition() {
        ...
        if (this.selection.startColumn != newSelection.startColumn) {
            const diff = newSelection.startColumn - this.selection.startColumn;
            this.selection = newSelection;
            this.domElement.style.left = `${this.domElement.offsetLeft + diff * this.editor.computeCharWidth(this.selection.startLineNumber)}px`;
        }
    ...
    scheduleCollisionCheck() {
        this.timer = setInterval(() => {
            if (this.editor) {
                ...
                if (x >= collisionElement.offsetLeft &&
...
export class PopUpMessage extends InlineMessage {
    ...
    createDomElement() {
        ...
        this.domElement.style.left = `${this.selection.startColumn * this.editor.computeCharWidth(this.code.getFirstLineNumber())}px`;
...
export class CodeBackground extends CodeHighlight {
    constructor(editor, statement) {
        ...
        if (statement.hasBody()) {
            ...
            statement.subscribe(CallbackType.replace, new Callback(() => {
                for (const line of statement.body) {
                    if (!line.background) {
                        line.background = new CodeBackground(editor, line);
            ...
            statement.subscribe(CallbackType.change, new Callback(() => {
                for (const line of statement.body) {
                    if (!line.background) {
                        line.background = new CodeBackground(editor, line);
                        line.background.updateDimensions();
            ...
            statement.subscribe(CallbackType.delete, new Callback(() => {
                for (const line of statement.body) {
                    if (line.background)
                        line.background.removeFromDOM();
                }
            }));
        }
    }
    ...
    updateDimensions(firstInsertion = false) {
        ...
        while (stack.length > 0) {
            ...
            if (lineDim.right > maxRight)
                maxRight = lineDim.right;
            if (line.hasBody())
                stack.unshift(...line.body);
        }
        width = maxRight - left;
        if (firstInsertion) {
            this.domElement.style.top = `${top}px`;
...
export class LineDimension {
    ...
    width;
    height;
    constructor(top, left, right, width, height) {
        this.top = top;
        this.left = left;
        this.right = right;
        this.width = width;
