...
(function (ErrorMessage) {
    ErrorMessage[ErrorMessage["general"] = 0] = "general";
    // this dependency in the future
    ErrorMessage[ErrorMessage["default"] = 1] = "default";
    ErrorMessage[ErrorMessage["outOfScopeVarReference"] = 2] = "outOfScopeVarReference";
    ErrorMessage[ErrorMessage["methodCallObjectTypeMismatch"] = 3] = "methodCallObjectTypeMismatch";
    ErrorMessage[ErrorMessage["binOpArgTypeMismatch"] = 4] = "binOpArgTypeMismatch";
    ErrorMessage[ErrorMessage["boolOpArgTypeMismatch"] = 5] = "boolOpArgTypeMismatch";
    ErrorMessage[ErrorMessage["compOpArgTypeMismatch"] = 6] = "compOpArgTypeMismatch";
    ErrorMessage[ErrorMessage["methodArgTypeMismatch"] = 7] = "methodArgTypeMismatch";
    ErrorMessage[ErrorMessage["addableTypeMismatchGeneral"] = 8] = "addableTypeMismatchGeneral";
    ErrorMessage[ErrorMessage["addableTypeMismatchControlStmt"] = 9] = "addableTypeMismatchControlStmt";
    ErrorMessage[ErrorMessage["addableTypeMismatchVarAssignStmt"] = 10] = "addableTypeMismatchVarAssignStmt";
    ErrorMessage[ErrorMessage["addableTypeMismatchEmptyLine"] = 11] = "addableTypeMismatchEmptyLine";
    ErrorMessage[ErrorMessage["existingIdentifier"] = 12] = "existingIdentifier";
    ErrorMessage[ErrorMessage["identifierIsKeyword"] = 13] = "identifierIsKeyword";
    ErrorMessage[ErrorMessage["identifierIsBuiltInFunc"] = 14] = "identifierIsBuiltInFunc";
    ErrorMessage[ErrorMessage["exprTypeMismatch"] = 15] = "exprTypeMismatch";
    ErrorMessage[ErrorMessage["addableTypeMismatchMethodArg"] = 16] = "addableTypeMismatchMethodArg";
    ErrorMessage[ErrorMessage["addableTypeMismatchOperator"] = 17] = "addableTypeMismatchOperator";
...
export class ErrorMessageGenerator {
     ...
     */
    generateMsg(errMsgType, args) {
        switch (errMsgType) {
            case ErrorMessage.outOfScopeVarReference:
                if (usePersonalizedMessages) {
                    return `I don't know what ${getStyledSpan(args.identifier, CSSClasses.identifier)} is. This could be because it has not been declared or it is declared on a line below this one.
            ...
            case ErrorMessage.methodCallObjectTypeMismatch:
                if (usePersonalizedMessages) {
                    return `I don't know how to do ${getStyledSpan(args.method, CSSClasses.identifier)}() from an
                            object of type ${getStyledSpan(args.calledOn, CSSClasses.type)}. I can only call it from objects of type 
            ...
            case ErrorMessage.binOpArgTypeMismatch:
                if (usePersonalizedMessages) {
                    return `I can only perform ${getStyledSpan(args.binOp, CSSClasses.other)} for like types. I cannot perform it on two different types
                    such as ${getStyledSpan(args.argType1, CSSClasses.type)} and ${getStyledSpan(args.argType2, CSSClasses.type)}.`;
                }
                return `${getStyledSpan(args.binOp, CSSClasses.other)} is not defined for types
                        ${getStyledSpan(args.argType1, CSSClasses.type)} and ${getStyledSpan(args.argType2, CSSClasses.type)}.`;
            case ErrorMessage.boolOpArgTypeMismatch:
                if (usePersonalizedMessages) {
                    `I cannot perform  ${getStyledSpan(args.binOp, CSSClasses.other)} on an expression of type ${getStyledSpan(args.argType2, CSSClasses.type)}.
                     I can only perform ${getStyledSpan(args.binOp, CSSClasses.other)} on ${getStyledSpan(args.argType1, CSSClasses.type)} literals, 
            ...
            case ErrorMessage.compOpArgTypeMismatch:
                if (usePersonalizedMessages) {
                    return `I cannot perform the comparison ${getStyledSpan(args.binOp, CSSClasses.other)} on different types such as 
                    ${getStyledSpan(args.argType1, CSSClasses.type)} and ${getStyledSpan(args.argType2, CSSClasses.type)}. They have to be the same type.`;
                }
                return `${getStyledSpan(args.binOp, CSSClasses.other)} is not defined for types ${getStyledSpan(args.argType1, CSSClasses.type)}
            ...
            case ErrorMessage.methodArgTypeMismatch:
                if (usePersonalizedMessages) {
                    return `This argument to ${getStyledSpan(args.methodName, CSSClasses.identifier)} should be of type 
                            ${getStyledSpan(args.argType1, CSSClasses.type)}, but I found a ${getStyledSpan(args.argType2, CSSClasses.type)}.`;
                }
            ...
            case ErrorMessage.addableTypeMismatchControlStmt:
                if (usePersonalizedMessages) {
                    if (args.constructName != "for") {
                        return `${getStyledSpan(args.constructName, CSSClasses.keyword)} is a control flow statement.
                                I can only put ${getStyledSpan("Boolean", CSSClasses.type)} expressions or anything else that evaluates 
                                to a ${getStyledSpan("Boolean", CSSClasses.type)} value.`;
                    }
                    else {
                        if (args.focusedNode instanceof IdentifierTkn) {
                            return `${getStyledSpan(args.constructName, CSSClasses.keyword)} is a control flow statement that I can only put a range
                ...
                if (args.constructName != "for") {
                    return `${getStyledSpan(args.constructName, CSSClasses.keyword)} is a control flow statement. It only accepts ${getStyledSpan("Boolean", CSSClasses.type)}
                            expressions or method calls and literal values that evaluate to a ${getStyledSpan("Boolean", CSSClasses.type)}. Tried to insert a ${getStyledSpan(args.addedType, CSSClasses.type)} instead.`;
                }
                else {
                    if (args.focusedNode instanceof IdentifierTkn) {
                        ...
                        return `Iterable object expected. ${getStyledSpan(args.addedType, CSSClasses.type)} found instead.`;
                    }
                    break;
                }
            case ErrorMessage.addableTypeMismatchVarAssignStmt:
                ...
                return `${getStyledSpan(args.constructName, CSSClasses.keyword)} accepts only text for the variable name. Tried to insert a
                        ${getStyledSpan(args.addedType, CSSClasses.keyword)} instead.`;
            case ErrorMessage.addableTypeMismatchEmptyLine:
                if (usePersonalizedMessages) {
                    return `I cannot insert a(n) ${getStyledSpan(args.addedType, CSSClasses.keyword)} on an empty line.`;
                }
            ...
            case ErrorMessage.existingIdentifier:
                if (usePersonalizedMessages) {
                    return `I am already using ${getStyledSpan(args.identifier, CSSClasses.identifier)} somewhere else. Please choose a different name.`;
                }
                return `The identifier ${getStyledSpan(args.identifier, CSSClasses.identifier)} is already in use in this scope.`;
            case ErrorMessage.identifierIsKeyword:
                if (usePersonalizedMessages) {
                    return `${getStyledSpan(args.identifier, CSSClasses.identifier)} is a special keyword that I am not allowed to use as a name for a variable.
                            Please try a different one.`;
                }
                return `The identifier ${getStyledSpan(args.identifier, CSSClasses.identifier)} is a reserved keyword. Please use a different one.`;
            case ErrorMessage.identifierIsBuiltInFunc:
                if (usePersonalizedMessages) {
                    return `I am already using ${getStyledSpan(args.identifier, CSSClasses.identifier)} as a name for a built-in function and it would
                            be difficult for me to keep track of another ${getStyledSpan(args.identifier, CSSClasses.identifier)}. Please choose a different name.`;
                }
                return `The identifier ${getStyledSpan(args.identifier, CSSClasses.identifier)} is the name of a built-in function or variable.
                        Please use a different one.`;
            case ErrorMessage.exprTypeMismatch:
                if (usePersonalizedMessages) {
                    return `I can only use ${getStyledSpan(args.expectedType, CSSClasses.type)} expressions here, but I found a
                            ${getStyledSpan(args.addedType, CSSClasses.type)} instead.`;
                }
                return `A(n) ${getStyledSpan(args.constructName, CSSClasses.keyword)} statement only accepts ${getStyledSpan(args.expectedType, CSSClasses.type)}
                        expressions. Found ${getStyledSpan(args.addedType, CSSClasses.type)} instead.`;
            case ErrorMessage.addableTypeMismatchMethodArg:
                if (usePersonalizedMessages) {
                    return `I cannot give ${getStyledSpan(args.addedType, CSSClasses.type)} to methods as they don't know how to work with it.`;
                }
...
//# sourceMappingURL=error-msg-generator.js.map