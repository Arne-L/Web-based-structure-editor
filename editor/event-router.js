...
export class EventRouter {
     ...
     */
    getKeyAction(e, providedContext) {
        /**
        ...
        switch (e.key) {
            ...
            case KeyPress.ArrowLeft: {
                ...
                if (inTextEditMode) {
                    ...
                    if (e.shiftKey && e.ctrlKey)
                        return new EditAction(EditActionType.SelectToStart);
                    else if (e.shiftKey)
                        return new EditAction(EditActionType.SelectLeft);
                    else if (e.ctrlKey)
                        return new EditAction(EditActionType.MoveCursorStart);
                    else {
            ...
            case KeyPress.ArrowRight: {
                ...
                if (inTextEditMode) {
                    ...
                    // IS THIS EVEN WORKING? (for all cases...)
                    if (e.shiftKey && e.ctrlKey)
                        return new EditAction(EditActionType.SelectToEnd);
                    else if (e.shiftKey)
                        return new EditAction(EditActionType.SelectRight);
                    else if (e.ctrlKey)
                        return new EditAction(EditActionType.MoveCursorEnd);
                    else {
            ...
            case KeyPress.Backspace: {
                ...
                if (curTkn instanceof ast.Token) {
                    if (curTkn instanceof ast.AutocompleteTkn ||
                        ...
                        if (curTkn.indexInRoot + nearestCompound.cycleLength >= nearestCompound.tokens.length &&
                            ...
                            let underNextCompound = nearestCompound;
                            while (!(underNextCompound.rootNode instanceof ast.CompoundConstruct)) {
                                underNextCompound = underNextCompound.rootNode;
                                if (!underNextCompound.rootNode)
                                    break;
                            }
                            const nextCompound = underNextCompound.rootNode;
                ...
                if (this.module.validator.canDeleteAdjacentChar(context, { backwards: true })) {
                    ...
                    if (e.ctrlKey)
                        return new EditAction(EditActionType.DeleteToStart);
                    else
                        return new EditAction(EditActionType.DeletePrevChar);
                }
            ...
            case KeyPress.Enter: {
                ...
                if (this.module.menuController.isMenuOpen())
                    return new EditAction(EditActionType.SelectMenuSuggestion);
            ...
            default: {
                ...
                if (inTextEditMode) {
                    ...
                    switch (e.key) {
                        ...
                        case KeyPress.C:
                            if (e.ctrlKey)
                                return new EditAction(EditActionType.Copy);
                            break;
                        case KeyPress.V:
                            if (e.ctrlKey)
                                return new EditAction(EditActionType.Paste);
                            break;
                        case KeyPress.Z:
                            if (e.ctrlKey)
                                return new EditAction(EditActionType.Undo);
                            break;
                        case KeyPress.Y:
                            if (e.ctrlKey)
                                return new EditAction(EditActionType.Redo);
                            break;
                    }
                    ...
                    if (token instanceof ast.IdentifierTkn && token.isEmptyIdentifier()) {
                        ...
                        curText.splice(this.curPosition.column - token.leftCol, Math.abs(selectedText.startColumn - selectedText.endColumn), e.key);
                        newText = curText.join("");
                    }
                    // Either it is not an identifier or editable text token OR the given regex matches the new text
                    if (!(editableTkn instanceof ast.IdentifierTkn || editableTkn instanceof ast.EditableTextTkn) ||
                ...
                let leftConstruct = context.tokenToLeft;
                // console.log("LeftConstruct", leftConstruct, context);
                let rightConstruct = null;
                while (leftConstruct?.right.equals(this.curPosition) &&
                    ...
                    if (rightConstruct instanceof ast.CompoundConstruct &&
    ...
    onButtonDown(id) {
        ...
        document.getElementById("toolbox-search-box").value = "";
        // Update the toolbox
     ...
     * @param id - The id of the button that was clicked
     */
    incrementButtonClicks(id) {
        // If the button has a count, increment the number of clicks
        if (this.buttonClicksCount.has(id))
            this.buttonClicksCount.set(id, this.buttonClicksCount.get(id) + 1);
        // Else set the number of clicks to 1
        else
            this.buttonClicksCount.set(id, 1);
     ...
     */
    routeToolboxEvents(e, context, source) {
     ...
     */
    executeMatchOnNavigation(token, providedContext) {
        // Get the current context
        ...
        if (token && token instanceof ast.AutocompleteTkn) {
            ...
            if (match) {
                match.performAction(this.module.executer, this.module.eventRouter, context, { type: "autocomplete", precision: "1", length: match.matchString.length + 1 }, {
