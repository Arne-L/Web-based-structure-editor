...
import { CodeConstruct, EditableTextTkn, 
// EmptyOperatorTkn,
IdentifierTkn, HoleTkn, } from "../syntax-tree/ast";
...
export class Editor {
    ...
    mousePosWindow = [0, 0];
    scrollOffsetTop = 0;
    oldCursorLineNumber = 1;
    mousePosMonaco;
    constructor(parentEl, module) {
     ...
     */
    executeEdits(range, code, overwrite = null) {
        // Text to use in the given range
        let text = overwrite;
        // If overwrite is null, use the code's render text
        if (overwrite == null)
            text = code?.getRenderText() ? code.getRenderText() : "";
     ...
     */
    indentRecursively(statement, { backward = false }) {
        // Indent the given statement to the left or right
        this.module.editor.executeEdits(new Range(statement.lineNumber, statement.leftCol, statement.lineNumber, statement.leftCol - (backward ? TAB_SPACES : 0)), null, backward ? "" : "    ");
        ...
        if (statement.hasBody()) {
            ...
            */
            while (stmtStack.length > 0) {
                ...
                this.module.editor.executeEdits(new Range(curStmt.lineNumber, curStmt.leftCol, curStmt.lineNumber, curStmt.leftCol - (backward ? TAB_SPACES : 0)), null, backward ? "" : "    ");
                if (curStmt.hasBody())
                    stmtStack.unshift(...curStmt.body);
            }
        }
    }
    insertAtCurPos(codeList) {
    ...
    computeCharWidthGlobal() {
        ...
        for (let i = 0; i < lines.children.length; i++) {
            const line = lines.children[i]?.children[0];
            if (line.getBoundingClientRect().width !== 0 && line.innerText.length !== 0) {
                return line.getBoundingClientRect().width / line.innerText.length;
            }
        }
    ...
    computeCharWidthInvisible(ln = 1) {
        let width = this.computeCharWidth(ln);
        if (width > 0)
            return width;
        const lines = Array.from(document.getElementsByClassName("view-lines")[0].children);
        for (const line of lines) {
            if (line.children[0] && line.children[0].innerText.length > 0) {
                return (line.children[0].getBoundingClientRect().width / line.children[0].innerText.length);
            }
        }
