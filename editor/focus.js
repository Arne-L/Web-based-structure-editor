...
import { AutocompleteTkn, CodeConstruct, CompoundConstruct, EditableTextTkn, EmptyLineStmt, GeneralExpression, GeneralStatement, IdentifierTkn, NonEditableTkn, Token, } from "../syntax-tree/ast";
...
export class Focus {
    ...
    onNavChangeCallbacks = new Array();
    onNavOffCallbacks = new Map();
    prevPosition = null;
    constructor(module) {
        this.module = module;
     ...
     */
    subscribeOnNavChangeCallback(callback) {
     ...
     */
    getContainingDraftNode(providedContext) {
        ...
        else if (focusedNode instanceof Token &&
     ...
     */
    getTextEditableItem(providedContext) {
        const context = providedContext ? providedContext : this.getContext();
        if (context.token instanceof IdentifierTkn ||
            context.token instanceof EditableTextTkn ||
            context.token instanceof AutocompleteTkn) {
            return context.token;
        }
        else if (context.tokenToLeft instanceof IdentifierTkn ||
    ...
    getContext(position) {
        ...
        if (!curSelection.getStartPosition().equals(curSelection.getEndPosition())) {
            ...
            context = this.getContextFromPosition(curLine, curPosition);
        context.position = curPosition;
        if (!context.codeConstruct)
            console.log("No code construct2", context.codeConstruct);
     ...
     */
    getFocusedStatement() {
        return this.getConstructAtPosition(this.module.editor.monaco.getPosition()); //this.getStatementAtLineNumber(this.module.editor.monaco.getPosition().lineNumber);
    }
    /**
    ...
    updateContext(newContext) {
        ...
        if (newContext.tokenToSelect != undefined) {
            const selection = new Selection(newContext.tokenToSelect.right.lineNumber, newContext.tokenToSelect.rightCol, newContext.tokenToSelect.left.lineNumber, newContext.tokenToSelect.leftCol);
     ...
     */
    navigatePos(pos, runNavOffCallbacks = true) {
            ...
            if (focusedToken instanceof Token && focusedToken.isEmpty) {
                ...
                if (hitDistance < tokenLength / 2) {
                    ...
                    if (focusedConstruct.left.isBefore(focusedToken.left.delta(undefined, -1))) {
                        const tokenBefore = this.getTokenAtCodeConstructPos(focusedConstruct, focusedToken.left.delta(undefined, -1));
                    ...
                    const tokenAfter = this.getTokenAtCodeConstructPos(focusedConstruct, focusedToken.right.delta(undefined, 1));
        ...
        if (runNavOffCallbacks && this.prevPosition != null) {
            const prevConstr = this.getConstructAtPosition(this.prevPosition), currConstr = this.getConstructAtPosition(curPos);
            if (prevConstr !== currConstr)
                this.fireOnNavOffCallbacks(prevConstr, currConstr);
        }
    ...
    navigateUp() {
        ...
        const focusedLineStatement = this.getConstructAtPosition(curPosition); //this.getStatementAtLineNumber(curPosition.lineNumber);
        const lineAbove = this.getConstructAtPosition(curPosition.delta(-1)); //this.getStatementAtLineNumber(curPosition.lineNumber - 1);
        if (focusedLineStatement !== lineAbove)
            this.fireOnNavOffCallbacks(focusedLineStatement, lineAbove);
        if (curPosition.lineNumber > 1)
            this.navigatePos(curPosition.delta(-1), false);
        else {
    ...
    navigateDown() {
        ...
        if (focusedLineStatement !== lineBelow)
    ...
    navigateRight() {
        ...
        this.fireOnNavOffCallbacks(focusedLineStatement, this.getConstructAtPosition(this.module.editor.monaco.getPosition()));
        this.fireOnNavChangeCallbacks();
    }
    /**
        ...
        if (this.onBeginningOfLine()) {
            ...
            if (!curSelection.getEndPosition().equals(curSelection.getStartPosition()) &&
                ...
                if (tokenBefore instanceof NonEditableTkn /*|| tokenBefore instanceof OperatorTkn*/) {
                    ...
                    const tokenBeforeBefore = this.getTokenAtCodeConstructPos(focusedLineStatement, tokenBefore.left.delta(undefined, -1));
                    if (tokenBeforeBefore instanceof Token && tokenBeforeBefore.isEmpty) {
                        this.selectCode(tokenBeforeBefore);
                    }
                    else if (tokenBeforeBefore instanceof Token &&
                        (tokenBeforeBefore instanceof EditableTextTkn || tokenBeforeBefore instanceof IdentifierTkn)) {
                        this.module.editor.monaco.setPosition(new Position(curPos.lineNumber, tokenBeforeBefore.rightCol));
                        this.module.editor.cursor.setSelection(null);
                    }
                    else {
     ...
     */
    isTextEditable(providedContext) {
        const context = providedContext ? providedContext : this.getContext();
        return ((context.token != null && context.token.isTextEditable) ||
    ...
    onEndOfLine() {
        ...
        if (curSelection.startColumn == curSelection.endColumn) {
            ...
            const focusedLineStatement = this.getConstructAtPosition(curPosition);
            if (focusedLineStatement != null && curPosition.column == focusedLineStatement.rightCol)
                return true;
        }
        return false;
    }
    ...
    onBeginningOfLine() {
        ...
        if (curSelection.startColumn == curSelection.endColumn) {
            const curPosition = curSelection.getStartPosition();
            const focusedLineStatement = this.getConstructAtPosition(curPosition);
            if (focusedLineStatement != null && curPosition.column == focusedLineStatement.leftCol)
                return true;
        }
        return false;
    }
     ...
     */
    getTokenAtCodeConstructPos(statement, pos) {
        const tokensStack = new Array();
        if (statement && statement instanceof CodeConstruct)
            tokensStack.unshift(...statement.tokens);
        while (tokensStack.length > 0) {
            const curToken = tokensStack.pop();
            if (!doesConstructContainPos(curToken, pos, { left: true, right: false }))
                continue;
     ...
     */
    fireOnNavOffCallbacks(oldStatement, newStatement) {
        const context = this.getContext();
        if (oldStatement && oldStatement !== newStatement) {
            ...
            const callbackArr = this.onNavOffCallbacks.get(ConstructName.Default /*oldStatement.codeConstructName*/) ?? [];
     ...
     */
    getCodeConstructAtLineNumber(line) {
        ...
        while (bodyStack.length > 0) {
            ...
            if (!(curStmt instanceof CodeConstruct))
                continue;
            // TODO: Could be multiple constructs on the same line, how to fix this?
            if (curStmt.left.lineNumber <= line && line <= curStmt.right.lineNumber)
                return curStmt;
            else
                bodyStack.unshift(...curStmt.tokens);
     ...
     * @returns the Statement object of that line.
     */
    getConstructAtPosition(position) {
        const bodyStack = [this.module.compoundConstruct];
        while (bodyStack.length > 0) {
            ...
            if (!doesConstructContainPos(curStmt, position))
                continue;
     ...
     */
    getContextFromSelection(statement, left, right) {
        const context = new Context();
        context.codeConstruct = statement;
        const tokensStack = new Array();
        // initialize tokensStack
        if (statement instanceof CodeConstruct)
            tokensStack.unshift(...statement.tokens);
        while (tokensStack.length > 0) {
            const curToken = tokensStack.pop();
            if (curToken.left.equals(left) && curToken.right.equals(right)) {
                ...
                if (curToken.tokens.length > 0)
                    tokensStack.unshift(...curToken.tokens);
                else {
                    console.warn(`getContextFromSelection(statement: ${statement}, left: ${left}, right: ${right}) -> found expression with no child tokens.`);
                }
            }
        }
        return context;
     ...
     */
    findNonTextualHole(statement, pos) {
        const tokensStack = new Array();
        for (const token of statement.tokens)
            tokensStack.unshift(token); // One liner: tokensStack.unshift(...statement.tokens);?
        while (tokensStack.length > 0) {
            const curToken = tokensStack.pop();
            ...
            if (curToken instanceof GeneralExpression)
                if (curToken.tokens.length > 0)
                    for (let token of curToken.tokens)
     ...
     */
    getContextFromPosition(statement, pos) {
        ...
        while (tokensStack.length > 0) {
            ...
            if (curToken instanceof Token) {
                ...
                if (pos.equals(curToken.left)) {
                    ...
                    context.tokenToLeft = this.searchNonEmptyTokenWithCheck(statement, (token) => token.right.equals(pos));
                    ...
                    context.tokenToRight = this.searchNonEmptyTokenWithCheck(statement, (token) => token.left.equals(pos));
                    if (context.tokenToRight != null) {
                        context.codeConstructToRight = context.tokenToRight.rootNode;
                ...
                if (curToken.tokens.length > 0)
                    ...
                    console.warn(`getContextFromPosition(statement: ${statement}, column: ${pos}) -> found expression with no child tokens.`);
                }
            }
        }
        return context;
     ...
     */
    getExpression(token, check) {
        if (token.rootNode instanceof GeneralExpression && check)
            return token.rootNode;
        return null;
    }
    /**
     * Searches the tokens tree for a token that matches the passed check() condition.
     */
    searchNonEmptyTokenWithCheck(statement, check) {
        const tokensStack = new Array();
        tokensStack.unshift(...statement.tokens);
        while (tokensStack.length > 0) {
            const curToken = tokensStack.pop();
            if (curToken instanceof Token && !curToken.left.equals(curToken.right) && check(curToken))
                return curToken;
...
export class Context {
    ...
    construct = null;
    codeConstructToLeft = null;
    codeConstructToRight = null;
    codeConstruct;
    selected = false; //this should not be nullable
    position = null;
    getAutocompleteToken() {
        if (this.token instanceof AutocompleteTkn)
            return this.token;
        else if (this.tokenToLeft instanceof AutocompleteTkn)
            return this.tokenToLeft;
        else if (this.tokenToRight instanceof AutocompleteTkn)
            return this.tokenToRight;
        else
            return null;
    }
}
export class UpdatableContext {
    tokenToSelect;
    positionToMove;
    constructor(tokenToSelect, positionToMove) {
