...
import { Expression, Modifier, Statement /*VariableReferenceExpr*/ } from "../syntax-tree/ast";
...
export const docBoxRunButtons = new Map(); // Can remove export
/**
...
export class ToolboxController {
    ...
    updateButtonsOnContextChange() {
        this.module.focus.subscribeOnNavChangeCallback(((c) => {
     ...
     */
    static updateButtonsVisualMode(insertionRecords) {
        for (const insertionRecord of insertionRecords) {
            const button = document.getElementById(insertionRecord.cssId);
            if (button) {
    ...
    loadToolboxFromJson() {
        ...
        staticDummySpace.style.minHeight = `${toolboxDiv.clientHeight - toolboxDiv.children[toolboxDiv.children.length - 2].clientHeight - 20}px`;
    }
}
/**
...
export class ToolboxButton {
     ...
     */
    static createToolboxButtonFromJsonObj(action) {
     ...
     */
    divButtonVisualMode(insertionType) {
 ...
 */
function constructCascadedMenuObj(validActions, buttonId, module, identifier, type) {
    ...
    const createExtraItem = (innerHTML) => {
    ...
    if (type === DataType.String) {
        ...
        extraContainer.appendChild(createExtraItem(`try converting to integer (number) using <span class='code'>int(<span class='inline-var'>${identifier}</span>)</span>`));
        menu.appendChild(extraContainer);
    }
    else if (type === DataType.Number) {
        extraContainer.appendChild(createExtraItem(`convert to string (text) using <span class='code'>str(<span class='inline-var'>${identifier}</span>)</span>`));
        menu.appendChild(extraContainer);
    }
    return menu;
}
 ...
 * @param type - The type of the variable, e.g. DataType.Number.
 */
function createAndAttachCascadedMenu(moreActionsButton, buttonId, validActions, module, identifier, type) {
    const button = document.getElementById(buttonId);
...
window.onresize = () => {
    const staticDummySpace = document.getElementById("static-toolbox-dummy-space");
    staticDummySpace.style.minHeight = `${staticDummySpace.parentElement.clientHeight -
 ...
 */
function addClassToButton(buttonId, className) {
    const button = document.getElementById(buttonId);
    if (button) {
...
class UseCaseSliderComponent {
    element;
    sendUsage;
    constructor(useCase, buttonId) {
        this.element = this.createUseCaseComponent(useCase.path, useCase.max, useCase.extension, useCase.prefix, useCase.explanations, useCase.title, useCase.id, buttonId);
    }
    /**
     ...
     */
    createUseCaseComponent(path, max, extension, prefix, explanations, title, id, buttonId) {
        ...
        for (let i = 1; i < max + 1; i++) {
            if (prefix)
                slides.push(`${path}${prefix}${i}.${extension}`);
            else
        ...
        this.sendUsage = () => {
            if (useCaseUsed) {
                Logger.Instance().queueEvent(new LogEvent(LogType.UseCaseSlideUsage, {
                    "use-case": id,
                    "button-id": buttonId,
...
export class TooltipComponent {
        ...
        if (code.documentation.tips) {
            ...
            for (const tip of code.documentation.tips) {
                ...
                if (tip.type == "use-case") {
                    ...
                    accordion.addRow(TooltipType.RunnableExample, tip.title ? tip.title : "run this example", ex[0], () => {
    ...
    onRemove() {
        if (!this.sentUsage) {
            ...
            if (duration > 3000) {
                Logger.Instance().queueEvent(new LogEvent(LogType.TooltipHoverDuration, {
