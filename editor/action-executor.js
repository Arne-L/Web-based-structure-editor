...
import { 
// AssignmentModifier,
AutocompleteTkn, GeneralStatement, IdentifierTkn, NonEditableTkn, 
// OperatorTkn,
Statement, TemporaryConstruct, Token, HoleTkn, } from "../syntax-tree/ast";
...
import { InsertionResult } from "./action-filter";
...
export class ActionExecutor {
     ...
     */
    execute(action, providedContext, e) {
        ...
        if (action?.data?.autocompleteData)
            flashGreen = true;
        ...
        switch (action.type) {
            ...
            case EditActionType.InsertGeneralExpr:
                ...
                if (flashGreen)
                    this.flashGreen(/*action.data?.statement*/ codeConstruct);
            ...
            case EditActionType.DeleteRootOfToken: {
                ...
                if (action.data?.backwards)
                    token = context.token ?? context.tokenToLeft;
                else
                    token = context.token ?? context.tokenToRight;
            ...
            case EditActionType.DeleteEmptyLine: {
                ...
                if (action.data?.pressedBackspace) {
                    ...
                    range = new Range(context.codeConstruct.lineNumber, context.codeConstruct.leftCol, lineAbove.lineNumber, lineAbove.rightCol);
                }
                else {
                    range = new Range(context.codeConstruct.lineNumber, context.codeConstruct.leftCol, context.codeConstruct.lineNumber + 1, context.codeConstruct.leftCol);
                }
            ...
            case EditActionType.DeletePrevLine: {
                ...
                const deleteRange = new Range(prevLine.lineNumber, prevLine.leftCol, prevLine.lineNumber + 1, prevLine.leftCol);
            ...
            case EditActionType.InsertChar: {
                ...
                if (token instanceof IdentifierTkn && token.isEmptyIdentifier()) {
                    ...
                    curText.splice(cursorPos.column - token.leftCol, Math.abs(selectedText.startColumn - selectedText.endColumn), pressedKey);
                ...
                if (selectedText.startColumn != selectedText.endColumn) {
                    editRange = new Range(cursorPos.lineNumber, selectedText.startColumn, cursorPos.lineNumber, selectedText.endColumn);
                    // If the token to the right is an identifier and empty
                }
                else if (context.tokenToRight?.isTextEditable &&
                    context.tokenToRight instanceof IdentifierTkn &&
                    context.tokenToRight.isEmpty) {
                    // Select the token for the given range
                    editRange = new Range(cursorPos.lineNumber, context.tokenToRight.leftCol, cursorPos.lineNumber, context.tokenToRight.rightCol);
                }
                else {
                    // Otherwise make an empty range for the given cursor position
                    editRange = new Range(cursorPos.lineNumber, cursorPos.column, cursorPos.lineNumber, cursorPos.column);
                }
            ...
            case EditActionType.DeleteNextChar: {
                ...
                const toDeleteItems = selectedText.startColumn == selectedText.endColumn
                    ? 1
                ...
                curText.splice(Math.min(cursorPos.column - token.leftCol - toDeletePos, selectedText.startColumn - token.leftCol - toDeletePos), toDeleteItems);
                newText = curText.join("");
                // Check against reserved words
                this.validateIdentifier(context, newText);
                // Check if it needs to turn back into a hole:
                if (newText.length == 0) {
                    let removableExpr = null;
                    // If the current expression is atomic (has no subexpressions or editable token)
                    if (context.construct instanceof GeneralStatement &&
                    ...
                    if (removableExpr != null) {
                        if (removableExpr instanceof AutocompleteTkn &&
                        ...
                        else if (removableExpr instanceof AutocompleteTkn &&
                            ...
                            this.module.deleteConstruct(removableExpr); //this.module.deleteCode(removableExpr);
                        break;
                    }
                    let identifier = null;
                    ...
                    if (identifier != null) {
                        ...
                        this.module.editor.executeEdits(new Range(cursorPos.lineNumber, identifier.leftCol, cursorPos.lineNumber, identifier.rightCol), null, "  ");
                        // rebuild ast
                ...
                // Monaco editor
                if (editableToken.setEditedText(newText)) {
                    let editRange = new Range(cursorPos.lineNumber, cursorPos.column, cursorPos.lineNumber, cursorPos.column);
                    if (selectedText.startColumn != selectedText.endColumn) {
                        editRange = new Range(cursorPos.lineNumber, selectedText.startColumn, cursorPos.lineNumber, selectedText.endColumn - 1);
                    }
            ...
            case EditActionType.InsertOperatorTkn: {
                ...
                // }
                if (flashGreen)
                    this.flashGreen(action.data.operator);
                break;
            }
     ...
     */
    deleteAutocompleteOnMatch(context) {
        // Get the current (autocomplete) token
        let token;
        // If the current focus is on or near an autocomplete token, set that token as the
        // current token
        if (context.token instanceof AutocompleteTkn)
            token = context.token;
        if (context.tokenToLeft instanceof AutocompleteTkn)
            token = context.tokenToLeft;
        if (context.tokenToRight instanceof AutocompleteTkn)
    ...
    performMatchAction(match, token) {
        ...
        // Otherwise, get the length of the match string
            ...
            length = match.matchString.length + 1;
        match.performAction(this, this.module.eventRouter, this.module.focus.getContext(), { type: "autocomplete", precision: "1", length }, {
    ...
    insertExpression(context, code) {
        ...
        // Current range
        const range = new Range(context.position.lineNumber, context.token.leftCol, context.position.lineNumber, context.token.rightCol);
     ...
     */
    checkImports(insertedCode, currentInsertionType) {
        // If invalid, don't check for imports
        if (currentInsertionType === InsertionType.Invalid)
            return;
    ...
    replaceCode(code, replace) {
        ...
        if (root instanceof Statement) {
            ...
            if (replace instanceof Token && replace.isEmpty) {
                ...
                this.module.focus.updateContext({ positionToMove: replace.getRightPosition() });
        }
    }
    // /**
     ...
     */
    validateIdentifier(context, identifierText) {
        ...
        if (focusedNode instanceof IdentifierTkn
        // ||
        // context.tokenToLeft instanceof IdentifierTkn ||
        // context.tokenToRight instanceof IdentifierTkn
        ) {
            if (Object.keys(PythonKeywords).indexOf(identifierText) > -1) {
                this.module.messageController.addPopUpMessage(focusedNode, { identifier: identifierText }, ErrorMessage.identifierIsKeyword);
            }
            else if (Object.keys(BuiltInFunctions).indexOf(identifierText) > -1) {
                this.module.messageController.addPopUpMessage(focusedNode, { identifier: identifierText }, ErrorMessage.identifierIsBuiltInFunc);
            }
     ...
     */
    updateAutocompleteMenu(autocompleteTkn) {
        this.module.menuController.updateMenuOptions(autocompleteTkn.getEditableText());
        this.module.menuController.updatePosition(this.module.menuController.getNewMenuPositionFromCode(autocompleteTkn));
    }
    // /**
     ...
     */
    openSuggestionMenu(context, text, autocompleteType) {
        /**
         * Central idea:
         * There are two options: either the menu should be opened somewhere were there is
         * not yet any autocomplete token or there is already an autocomplete token present.
        ...
        if (existingAutocompleteTkn instanceof AutocompleteTkn) {
            ...
            autocompleteTkn.subscribe(CallbackType.change, new Callback((() => {
                if (!this.module.menuController.isMenuOpen()) {
            ...
            if (match) {
                ...
                autocompleteTkn.subscribe(CallbackType.delete, new Callback(() => {
                    if (highlight) {
                        highlight.removeFromDOM();
                        highlight = null;
                    }
