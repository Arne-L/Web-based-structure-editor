import { 
// Expression,
GeneralExpression, } from "../syntax-tree/ast";
...
import { Actions, EditActionType } from "./consts";
...
export class ActionFilter {
        ...
        for (const action of Actions.instance().actionsList) {
            if (action.referenceType) {
                ...
                for (const reference of references) {
                    ...
                    validOptionMap.set(Math.random().toString(36).substring(8), // Key is useless, thus should simply be unique
                    EditCodeAction.createDynamicEditCodeAction(action.optionName, // Does this need to be changed? When is this used?
                    action.cssId, () => action.getCodeFunction({ reference: reference.getAssignment().getRenderText() }), action.insertActionType, action.insertData, action.validateAction(this.module.validator, context), action.terminatingChars, matchTxt, regexTxt, action.insertableTerminatingCharRegex));
                }
            }
            else {
                validOptionMap.set(action.optionName, EditCodeAction.createDynamicEditCodeAction(action.optionName, action.cssId, action.getCodeFunction, action.insertActionType, action.insertData, action.validateAction(this.module.validator, context), action.terminatingChars, action.matchString, action.matchRegex, action.insertableTerminatingCharRegex));
            }
        }
     ...
     */
    getProcessedConstructInsertions() {
     ...
     */
    convertInsertionMapToList(insertionMap) {
        // const inserts = [];
...
export class UserAction {
    ...
    optionName;
    cssId;
    constructor(optionName, cssId) {
        this.optionName = optionName;
        this.cssId = cssId;
    }
    validateAction(validator, context) {
        return new InsertionResult(InsertionType.Invalid, "", []);
    }
    performAction(executor, eventRouter, context, source) { }
}
export class EditCodeAction extends UserAction {
     ...
     */
    constructor(optionName, cssId, getCodeFunction, insertActionType, insertData = {}, documentation, terminatingChars, matchString, matchRegex, insertableTerminatingCharRegex, trimSpacesBeforeTermChar = false) {
    ...
    static createDynamicEditCodeAction(optionName, cssId, getCodeFunction, insertActionType, insertData = {}, insertionResult, // Determines if the EditCodeAction is valid or not, aka disabled in the toolbox or not
    terminatingChars, matchString, matchRegex, insertableTerminatingCharRegex) {
        const action = new EditCodeAction(optionName, cssId, getCodeFunction, insertActionType, insertData, null, terminatingChars, matchString, matchRegex, insertableTerminatingCharRegex);
    ...
    performAction(executor, eventRouter, providedContext, source, autocompleteData) {
        // Current context
        let context = providedContext;
        if (autocompleteData)
            context = executor.deleteAutocompleteOnMatch(providedContext);
        const editAction = eventRouter.routeToolboxEvents(this, context, source);
        if (editAction.data)
            editAction.data.autocompleteData = autocompleteData;
        else
            editAction.data = { autocompleteData };
...
export class InsertionResult {
    ...
    constructor(insertionType, msg, typeConversionRecord) {
        this.insertionType = insertionType;
        this.message = msg;
        // this.conversionRecords = typeConversionRecord;
    }
}
//# sourceMappingURL=action-filter.js.map