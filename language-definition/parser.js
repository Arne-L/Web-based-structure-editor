...
import { GeneralExpression, GeneralStatement } from "../syntax-tree/ast";
import config from "./config.json";
// Dynamically import the correct language and constructs
let languageConfig;
if (config.languageFile)
    languageConfig = (await import(`../language-definition/${config.languageFile}`)).default;
else
    throw new Error("The language-file field is not correctly specified in the configuration file");
let constructs;
if (languageConfig.constructFile)
    constructs = (await import(`../language-definition/${languageConfig.constructFile}`)).default;
else
    throw new Error("No construct file specified in the language configuration file");
let recursiveFormats;
if (languageConfig.callableFile)
    recursiveFormats = (await import(`../language-definition/${languageConfig.callableFile}`)).default;
else
    throw new Error("No recursive file specified in the language configuration file");
...
export function getAllCodeActions() {
    ...
    const flattenedConstructs = []; // TODO: Replace the any type
    ...
    for (const construct of flattenedConstructs) {
        const action = new EditCodeAction(construct.editorName, `add-${construct.keyword}-btn-${toolboxId++}`, getCodeFunction(construct), construct.constructType === "expression"
        ...
        construct.toolbox, construct.triggerInsertion ?? [], // EXTRACT: character which triggers the insertion in the editor
        ...
        action.referenceType = construct.format.find((struct) => struct.type === "reference")?.to;
    ...
    GeneralStatement.addAllConstructs(editCodeActions.map((action) => action.getCode()));
    /**
 ...
 */
export function addEditCodeActionsToCategories(toolboxCategories, editCodeActions) {
    for (const action of editCodeActions) {
        const currentCategory = action.documentation.category;
        if (toolboxCategories.some((category) => currentCategory === category.displayName)) {
            ...
            const newCategory = new ToolboxCategory(currentCategory, currentCategory.toLowerCase() + "-ops-toolbox-group", [action]);
 ...
 */
function getCodeFunction(construct) {
    // Currently handle expression and statement separately
    // Merge them into one in the future
    if (construct.constructType === "expression")
        return (data) => new GeneralExpression(construct, null, null, data);
    else
        return (data) => new GeneralStatement(construct, null, null, data);
}
export function initLanguage() {
    return {
        language: languageConfig.name,
        reservedWords: new Map((languageConfig.reservedWords ?? []).map((reservedCategory) => [
