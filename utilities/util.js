import { AssignmentTkn, EditableTextTkn } from "../syntax-tree/ast";
...
export class Util {
    ...
    constructor(module) {
        ...
        this.typeConversionMap = new Map([
            [DataType.Void, []],
...
export function areEqualTypes(incoming, receiving) {
    if (receiving == DataType.Any)
        return true;
    else if (receiving == DataType.AnyList && ListTypes.indexOf(incoming) > -1)
        return true;
    else if (receiving == incoming)
        return true;
    return false;
}
/**
 ...
 */
export function hasMatch(list1, list2) {
 ...
 * // => [4, 3]
 */
export function hasMatchWithIndex(list1, list2) {
    const matchingIndices = [-1, -1];
 ...
 */
export function isImportable(object) {
    return Object.hasOwn(object, "requiredModule") && !!object.requiredModule; //calling hasOwnProperty with call() because 'object' is not necessarily an object
}
export function getUserFriendlyType(type) {
    switch (type) {
        case DataType.Any:
            return "any";
...
export function createWarningButton(buttonTxt, warningCode, action) {
 ...
 */
export function createFinalConstruct(action) {
    const construct = action.data?.construct;
    const autocompleteValues = action.data?.autocompleteData?.values;
    // Update the contents of the tokens
    let index = 0;
    for (const token of construct.tokens) {
        if ((token instanceof AssignmentTkn || token instanceof EditableTextTkn) &&
            autocompleteValues &&
 ...
 */
export function getHoleValues(userInput, regex) {
    if (!regex)
        return [];
    regex.lastIndex = 0;
 ...
 */
export var DOMManupulation;
(function (DOMManupulation) {
    function createElement(tag, classes, style, innerHTML, attributes, children) {
    ...
    DOMManupulation.createElement = createElement;
 ...
 */
export function doesConstructContainPos(construct, pos, inclusive = { left: true, right: true }) {
    return ((inclusive.left ? construct.left.isBeforeOrEqual(pos) : construct.left.isBefore(pos)) &&
