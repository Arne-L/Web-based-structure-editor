...
class Menu {
    ...
    linkMenuThroughOption(child, optionInParent) {
    ...
    static collapseSingleLinkMenus(root) {
    ...
    getPossibleEditCodeActions(userInput) {
        ...
        function sortActions(a, b) {
            ...
            if (a.matchString && b.matchRegex)
                return -1;
            if (a.matchRegex && b.matchString)
                return 1;
            // Get the final rendered text, as a string, for both options
            const aText = a.getConstructText(userInput).toLowerCase(), bText = b.getConstructText(userInput).toLowerCase();
            ...
            const aStart = aText.indexOf(lowUserInput), bStart = bText.indexOf(lowUserInput), aDiff = aText.length - userInput.length, bDiff = bText.length - userInput.length;
...
class MenuOption {
    ...
    selectAction;
    constructor(text = "Option Text", useInnerHTML = false, childMenu, parentMenu, doc, selectAction, extraInformation, draftMode = false) {
        this.text = text;
        ...
        if (draftMode)
            this.htmlElement.classList.add(MenuController.draftModeOptionElementClass);
        let textNode;
    ...
    select() {
        if (this.childMenu)
            this.childMenu.open(); // TODO: No child menu's anymore?
        else if (this.selectAction)
            this.selectAction();
    }
    linkToChildMenu(child) {
    ...
    setFocus() {
        ...
        if (this.childMenu) {
            this.childMenu.open();
            this.childMenu.htmlElement.style.top = `${this.htmlElement.offsetTop +
                this.parentMenu.htmlElement.offsetTop -
                this.parentMenu.htmlElement.scrollTop}px`;
        }
        else if (this.doc) {
...
export class MenuController {
    ...
    topOptionIndex = 0;
    bottomOptionIndex = 0;
    static suggestionOptionExtraInfo = "suggestionOptionExtraInfo";
    static optionElementClass = "suggestionOptionParent";
    static draftModeOptionElementClass = "draftModeOptionElementClass";
    static menuElementClass = "suggestionMenuParent";
    static optionTextElementClass = "suggestionOptionText";
    static selectedOptionElementClass = "selectedSuggestionOptionParent";
    oldScrollOffset = 0;
    module;
    editor;
    indexOfRootMenu = -1;
     ...
     */
    buildSingleLevelMenu(suggestions, pos = { left: 0, top: 0 }) {
     ...
     */
    calculateAutocompleteMatchPrecision(text, matchString) {
        if (matchString && text) {
            let matchCount = 0;
            for (let i = 0; i < text.length; i++) {
                if (text[i] === matchString[i])
                    matchCount++;
            }
     ...
     */
    buildMenu(options, pos = { left: 0, top: 0 }) {
        if (options.length > 0) {
            const menuOptions = new Map();
            for (const action of options) {
                menuOptions.set(action.optionName, () => {
                    action.performAction(this.module.executer, this.module.eventRouter, this.module.focus.getContext(), { type: "autocomplete-menu", precision: "0", length: action.matchString.length + 1 }, {});
                });
            }
            ...
            menu.htmlElement.style.top = `${pos.top + parseFloat(window.getComputedStyle(document.getElementById(EDITOR_DOM_ID)).paddingTop)}px`;
    ...
    focusOptionBelow() {
        ...
        if (this.focusedOptionIndex == 0) {
            ...
            this.menus[this.focusedMenuIndex].htmlElement.scrollTop += optionDomElements[0].offsetHeight;
        }
    ...
    focusOptionAbove() {
        ...
        if (this.focusedOptionIndex == options.length - 1) {
            this.menus[this.focusedMenuIndex].htmlElement.scrollTop =
                optionDomElements[0].offsetHeight * options.length;
    ...
    updateMenuArrayFromTree(root, isRoot) {
        if (isRoot) {
            this.indexOfRootMenu = 0;
            this.menus = [];
            this.focusedOptionIndex = 0; //TODO: If we ever go back to nested menus then this.topOptionIndex and this.bottomOptionIndex need to be updated here.
    ...
    styleMenuOptions() {
        if (this.isMenuOpen()) {
            ...
            for (const action of actionsToKeep) {
                ...
                let option = new MenuOption(action.optionName, true, null, menu, null, () => {
                    action.performAction(this.module.executer, this.module.eventRouter, this.module.focus.getContext(), "autocomplete-menu", {});
                }, null, action.insertionResult.insertionType == InsertionType.DraftMode);
                // Add option to menu
                menu.insertOption(option);
    ...
    updateMenuOptions(optionText) {
        ...
        for (const editAction of menuOptions) {
            ...
            if (code.containsAssignments() && this.module.language.isReservedWord(optionText))
                continue;
            // Create the HTML string for the option name
            const optionDisplayText = getStyledSpanAtSubstrings(editAction.getDisplayText(optionText), //code.getRenderText(),
            "matchingText", substringMatchRanges);
            ...
            if ((editAction.matchRegex || editAction.matchString === optionText) &&
                editAction.terminatingChars.length > 0) {
                extraInfo = `press <span class="highlighted-text">${this.convertTerminatingChar(editAction.terminatingChars[0])}</span> to insert`;
            }
            else {
            ...
            option = new MenuOption(optionDisplayText, true, null, menu, null, () => {
                editAction.performAction(this.module.executer, this.module.eventRouter, this.module.focus.getContext(), {
            ...
            }, extraInfo, editAction.insertionResult.insertionType == InsertionType.DraftMode);
            menu.insertOption(option);
        }
    ...
    getNewMenuPositionFromPosition(position) {
        ...
        pos.left =
            document.getElementById(EDITOR_DOM_ID).offsetLeft +
                document
                    ...
                    .getElementsByClassName("margin")[0].offsetWidth +
    ...
    getNewMenuPositionFromCode(code) {
        const pos = { left: 0, top: 0 };
        pos.left =
            document.getElementById(EDITOR_DOM_ID).offsetLeft +
                document
    ...
    updateFocusedMenuScroll(scrollOffset) {
        if (this.isMenuOpen()) {
            this.menus[this.focusedMenuIndex].htmlElement.style.top = `${this.menus[this.focusedMenuIndex].htmlElement.offsetTop + (this.oldScrollOffset - scrollOffset)}px`;
        }
        this.oldScrollOffset = scrollOffset;
    }
    hasNoSuggestions() {
        return (this.menus[this.focusedMenuIndex].options.length == 1 &&
            this.menus[this.focusedMenuIndex].options[0].text == "No suitable options found.");
    }
    convertTerminatingChar(text) {
        if (text == " ")
            return "space";
        else
            return text;
