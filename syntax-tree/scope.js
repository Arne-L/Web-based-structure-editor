...
export class Scope {
     ...
     */
    getValidReferences(pos, referenceType) {
        return [
            ...(referenceType
     ...
     */
    getValidReferencesRecursive(pos, referenceType) {
     ...
     */
    getAccessableAssignments(identifier, pos) {
    ...
    removeAssignment(assignment) {
        ...
        this.references.set(assignment.referenceType, references.filter((ref) => ref.getAssignment() !== assignment));
    ...
    addAssignment(assignment) {
        const references = this.references.get(assignment.referenceType);
        if (references)
            references.push(new Reference(assignment, this));
        else
            this.references.set(assignment.referenceType, [new Reference(assignment, this)]);
    }
    /**
     ...
     */
    pushToScope(toScope, assignments) {
        let total = 0;
        if (!toScope)
            return;
        for (let assignment of assignments) {
            ...
            if (removed)
                toScope.addAssignment(assignment);
            total += removed ? 1 : 0;
        }
        return total;
    }
