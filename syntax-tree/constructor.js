...
import { AssignmentTkn, CompoundConstruct, EditableTextTkn, NonEditableTkn, HoleTkn, } from "./ast";
export var SyntaxConstructor;
(function (SyntaxConstructor) {
    /**
     ...
     */
    function constructTokensFromJSON(formatTokens, rootConstruct, indexInRoot, data) {
        const constructs = [];
    ...
    SyntaxConstructor.constructTokensFromJSON = constructTokensFromJSON;
    /**
     * Construct an array of constructs from the given JSON specification.
     ...
     */
    function constructTokensFromJSONCompound(jsonConstruct, rootConstruct, data, startingConstructs, startingIndex, indexInRoot = 0, initialConstruction = false) {
        const constructs = [];
        ...
        if (initialConstruction && stopCondition(jsonConstruct.format[index]))
            return constructs;
        do {
        ...
        if (finalConstructs)
            finalConstructs.splice(indexInRoot, 0, ...constructs);
    ...
    SyntaxConstructor.constructTokensFromJSONCompound = constructTokensFromJSONCompound;
    /**
     ...
     */
    function stopCondition(token) {
        if ("waitOnUser" in token && token.waitOnUser)
            return true;
        return false;
    }
    /**
     ...
     */
    function constructToken(token, rootConstruct, indexInRoot, data) {
        switch (token.type) {
            case "token":
            ...
            case "hole":
                ...
                for (let i = 0; i < token.elements.length; i++) {
                    ...
                    constructs.push(new HoleTkn(rootConstruct, indexInRoot + constructs.length, token.elements[i].type));
            ...
            case "identifier":
                return [
                    new AssignmentTkn(undefined, rootConstruct, indexInRoot, RegExp(token.regex), token.scopeType, token.reference),
                ];
            case "reference":
