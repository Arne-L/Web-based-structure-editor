...
import { AutocompleteTkn, EmptyLineStmt, Expression, 
// FormattedStringCurlyBracketsExpr,
GeneralStatement, ImportStatement, 
// ListLiteralExpression,
Statement, Token, HoleTkn, } from "./ast";
import { rebuildBody } from "./body";
import { CallbackType } from "./callback";
import { SyntaxConstructor } from "./constructor";
import { MISSING_IMPORT_DRAFT_MODE_STR } from "./consts";
...
export class Module {
    ...
    constructor(editorId) {
        ...
        const range = new Range(this.compoundConstruct.left.lineNumber, this.compoundConstruct.leftCol, this.compoundConstruct.right.lineNumber, this.compoundConstruct.rightCol);
        ...
        this.focus.subscribeOnNavChangeCallback((c) => {
        ...
        this.focus.subscribeOnNavChangeCallback((c) => {
            ...
            if (!(c.token instanceof AutocompleteTkn ||
     ...
     */
    recursiveNotify(code, callbackType) {
        ...
        if (code instanceof Expression || code instanceof Statement) {
            ...
            while (codeStack.length > 0) {
                ...
                if (curCode instanceof Statement || curCode instanceof Expression)
                    codeStack.unshift(...curCode.tokens);
                if (curCode instanceof Statement && curCode.hasBody())
                    codeStack.unshift(...curCode.body);
            }
        ...
        if (root instanceof Statement) {
            ...
            if (!stmt.hasBody()) {
                ...
                while (stmtStack.length > 0) {
                    ...
                    if (curStmt.hasBody())
                        stmtStack.unshift(...curStmt.body);
                }
     ...
     */
    indentBodyConstructs(providedContext, backwards) {
        // The parent statement
     ...
     * @param backwards - Whether to indent backwards or forwards
     */
    indentConstruct(statement, backwards) {
     ...
     */
    deleteConstruct(construct, options = { executeEdit: true }) {
     ...
     */
    replaceFocusedExpression(construct) {
        // Current context
        ...
        let codeToReplace = /*context.codeconstruct ??*/ context.token; // Always deleting a EmptyTypedExpr token
        const root = codeToReplace.rootNode;
        root.replace(construct, codeToReplace.indexInRoot);
    }
    closeConstructDraftRecord(code) {
        if (code.draftModeEnabled) {
            ...
            if (removedRecord.warning)
                removedRecord.removeMessage();
    ...
    openDraftMode(code, txt = "Draft Mode Placeholder Txt", actionButtons) {
     ...
     */
    getCodeStatus(highlightConstructs = false) {
        let status = null;
        ...
        while (stack.length > 0) {
            ...
            if (cur instanceof HoleTkn /*&& !cur.isListElement()*/) {
                ...
                if (highlightConstructs && !hasDraftMode)
                    cur.addHighlight(ERROR_HIGHLIGHT_COLOUR, this.editor);
            }
                ...
                for (let i = 0; i < cur.tokens.length; i++) {
                    if (cur.tokens[i] instanceof HoleTkn &&
                        addHighlight &&
     ...
     */
    performActionOnBFS(duringAction) {
    ...
    openImportDraftMode(code) {
        ...
        button.addEventListener("click", (() => {
            this.executer.execute(new EditAction(EditActionType.InsertImportFromDraftMode, {
