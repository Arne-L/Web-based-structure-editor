import { CompoundConstruct, GeneralStatement, HoleTkn } from "./ast";
import { Module } from "./module";
export var ValidatorNameSpace;
(function (ValidatorNameSpace) {
    function validateRequiredConstructs(context, invokedConstruct) {
        // If there are no required constructs, the depending constructs are always valid
        if (invokedConstruct.requiredConstructs.length === 0)
            return true;
        ...
        for (const requiredName of invokedConstruct.requiredConstructs) {
            ...
            // TODO: See todo above
            let dependingIndex = depConstructsInfo.findIndex((construct) => construct.getConstructName() === invokedConstruct.getKeyword());
            ...
            if (currentConstruct instanceof CompoundConstruct) {
                ...
                if (replacement instanceof HoleTkn)
                    currentConstruct = replacement;
            }
            ...
            if (nextConstruct) {
                ...
                let nextIndex = depConstructsInfo.findIndex((construct) => construct.getConstructName() === nextConstruct.getKeyword());
                ...
                while (nextIndex >= dependingIndex) {
                    ...
                    if (!nextConstruct)
                        break;
                    nextIndex = depConstructsInfo.findIndex((construct) => construct.getConstructName() === nextConstruct.getKeyword());
                }
            }
    ...
    ValidatorNameSpace.validateRequiredConstructs = validateRequiredConstructs;
    function validateAncestors(context, invokedConstruct) {
        // If element needs to be a descendant of a certain construct
        if (invokedConstruct.requiredAncestorConstructs.length === 0)
            return true;
        let canInsertConstruct = false;
        ...
        let level = 0;
        while (currentParent) {
            const foundAncestor = invokedConstruct.requiredAncestorConstructs.find((ancestor) => ancestor.getConstructName() === currentParent.getKeyword());
    ...
    ValidatorNameSpace.validateAncestors = validateAncestors;
    /**
     ...
     */
    function getNextSiblingOf(construct) {
        ...
        if (!construct.rootNode || construct.indexInRoot === construct.rootNode.tokens.length - 1)
            return null;
        ...
        if (nextConstruct instanceof GeneralStatement || nextConstruct instanceof HoleTkn)
            return nextConstruct;
        // Otherwise, keep searching
        else
            return getNextSiblingOf(nextConstruct);
    }
    /**
     ...
     */
    function getPrevSiblingOf(construct) {
        // Construct is the first construct in the root's tokens
        if (!construct?.rootNode || construct.indexInRoot === 0)
            return null;
        ...
        if (prevConstruct instanceof GeneralStatement || prevConstruct instanceof HoleTkn)
            return prevConstruct;
        // Otherwise, keep searching
        else
            return getPrevSiblingOf(prevConstruct);
    }
    /**
     ...
     */
    function getParentOf(statement) {
        if (statement.rootNode instanceof Module)
            return null;
        return statement.rootNode;
    }
})(ValidatorNameSpace || (ValidatorNameSpace = {}));
